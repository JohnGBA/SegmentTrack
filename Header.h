...
void trackbar(void(*functocall)(int, void*), std::string image_window, std::string hist_window = NULL, std::string grad_window = NULL);

void otsu_trackbar(void(*functocall)(int, void*), std::string image_window);

void canny_with_otsu(cv::Mat& image);

void canny_normal(cv::Mat& image);

void drawBalls(cv::Mat& image, std::vector< std::vector<cv::Point> >& v);
